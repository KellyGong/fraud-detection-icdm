...
def read_node_atts(node_file, pyg_file, label_file=None):
    ...
    # homo_item_graph
    item_graph.x = torch.empty(len(node_maps['item']), 256)
    for nid, embedding in tqdm(node_embeds['item'].items()):
        item_graph.x[nid] = torch.from_numpy(embedding)
    item_graph.num_nodes = len(node_maps['item'])
    item_graph.maps = node_maps['item']
    ...
    if label_file is not None:
        item_graph.y = torch.zeros(len(node_maps['item']), dtype=torch.long) - 1
        for index, label in tqdm(labels, desc="Node labels"):
            item_graph.y[index] = label
        
        item_graph.train_idx = train_idx
        item_graph.val_idx = val_idx


    for ntype in graph.node_types:
        graph[ntype].n_id = torch.arange(graph[ntype].num_nodes)
    
    item_graph.n_id = torch.arange(graph['item'].num_nodes)
...
#        graph = torch.load(pyg_file + ".pt")
        ...
        print("##########################################\n")
        graph, item_graph = read_node_atts(node_file, pyg_file, label_file)
    ...
    with open(edge_file, 'r') as rf:
        while True:
            ...
            nodes_adj_nodes.setdefault(source_type, {})
            nodes_adj_nodes[source_type].setdefault(source_id, []).append([dest_type, dest_id])
    ...
    middle_type2int = {}
    source_dest2middle_type = {}
    
    for item_node in tqdm(nodes_adj_nodes['item'], total=len(nodes_adj_nodes['item']), desc='homo item select neighbor'):
        for dest_type, dest_id in nodes_adj_nodes['item'].setdefault(item_node, []):
            for hop_2_dest_type, hop_2_dest_id in nodes_adj_nodes[dest_type].setdefault(dest_id, []):
                if hop_2_dest_type == 'item':
                    # item_source.append(item_node)
                    # item_dest.append(hop_2_dest_id)
                    # edge_type.append(dest_type)
                    if dest_type not in middle_type2int:
                        middle_type2int[dest_type] = len(middle_type2int)
                    source_dest2middle_type.setdefault((item_node, hop_2_dest_id), []).append(middle_type2int[dest_type])
    
    num_middle_type = len(middle_type2int)

    for source, dest in source_dest2middle_type:
        item_source.append(source)
        item_dest.append(dest)

        middle_types = source_dest2middle_type[(source, dest)]
        middle_types_tensor = torch.zeros(num_middle_type, dtype=torch.long)
        for middle_type in middle_types:
            middle_types_tensor[middle_type] = 1
        
        edge_type.append(middle_types_tensor)

    item_source = torch.tensor(item_source, dtype=torch.long)
    item_dest = torch.tensor(item_dest, dtype=torch.long)
    item_graph.edge_index = torch.vstack([item_source, item_dest])
    item_graph.edge_attr = torch.cat(edge_type)
    ...
    print('Start saving into pyg data')
    torch.save(graph, pyg_file + ".pt")
    torch.save(item_graph, pyg_file + "_item.pt")
    print('Complete saving into pyg data\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--storefile', type=str, default="dataset/pyg_data/icdm2022_session1_homo")
