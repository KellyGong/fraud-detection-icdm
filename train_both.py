...
parser.add_argument('--dataset', type=str, default='dataset/pyg_data/icdm2022_session_both.pt')
...
converted_test_id = []
for i in test_id:
    converted_test_id.append(hgraph['item'].maps[i + node_size1])
...
###################### Stage 2 add a new edge_type item-I-item based on feature distance
def find_edge2add(hgraph, num_edge2add, idx1, idx2):
    origin_dist = torch.cdist(torch.unsqueeze(hgraph['item'].x[idx1], 0), torch.unsqueeze(hgraph['item'].x[idx2], 0)).squeeze(0)
    dist = torch.clone(origin_dist)
    # dist.fill_diagonal_(-1) # remove diagonal
    triu = False # compute whole matrix
    if (torch.numel(idx1) == torch.numel(idx2)):
        if torch.all(torch.eq(idx1, idx2)):
            triu = True # compute only upper triangular part
    if triu:
        dist[torch.triu(dist, diagonal=1)==0] = -1 # remove lower triangle
        dist[dist==0] = -1 # remove nodes that have all 0 feature
        dist = np.array(dist)
        ...
        threshold = np.partition(off_diag, num_edge2add//2)[:num_edge2add//2].max()
        edge2add_idx = np.where((dist<=threshold) & off_diag_ind)
        sources, targets = idx1[edge2add_idx[0]], idx2[edge2add_idx[1]]
        sources, targets = torch.cat([sources, targets]), torch.cat([targets, sources])
        dists = torch.tensor(np.concatenate([origin_dist[edge2add_idx], origin_dist[edge2add_idx[1], edge2add_idx[0]]]))
    else:
        dist[dist==0] = -1 # remove nodes that have all 0 feature
        dist = np.array(dist)
        no_0_ind = dist!=-1
        no_0 = dist[no_0_ind]
        threshold = np.partition(no_0, num_edge2add)[:num_edge2add].max()
        edge2add_idx = np.where((dist<=threshold) & no_0_ind)
        sources, targets = idx1[edge2add_idx[0]], idx2[edge2add_idx[1]]
        dists = origin_dist[edge2add_idx]
    return sources, targets, dists

def add_item_item_edge_separately(hgraph, num_edge2add, train_idx, val_idx, test_idx):
    start_time = time.time()
    all_size = torch.numel(train_idx) + torch.numel(val_idx) + torch.numel(test_idx)
    print('Adding {} edges item-I-item among {} nodes...'.format(num_edge2add, all_size))
    train_val_idx = torch.cat([train_idx, val_idx])
    sources1, targets1, dists1 = find_edge2add(hgraph, num_edge2add, train_val_idx, train_val_idx)
    sources2, targets2, dists2 = find_edge2add(hgraph, num_edge2add, train_val_idx, test_idx)
    sources3, targets3, dists3 = find_edge2add(hgraph, num_edge2add, test_idx, test_idx)
    threshold = np.partition(torch.cat([dists1, dists2, dists3]), num_edge2add)[:num_edge2add].max()
    edge2add_idx1, edge2add_idx2, edge2add_idx3 = np.where(dists1<=threshold), np.where(dists2<=threshold), np.where(dists3<=threshold)
    sources, targets = torch.cat([sources1[edge2add_idx1], sources2[edge2add_idx2], sources3[edge2add_idx3]]), torch.cat([targets1[edge2add_idx1], targets2[edge2add_idx2], targets3[edge2add_idx3]])
    hgraph['item', 'I', 'item'].edge_index = torch.vstack([sources, targets])
    print("It took {} minutes to add {} edges among {} nodes".format((time.time() - start_time)/60, hgraph['item', 'I', 'item'].edge_index.shape[1], all_size))
    return hgraph

if args.item_item_sepa:
    hgraph = add_item_item_edge_separately(hgraph, args.edge_add, train_idx, val_idx, test_idx)
